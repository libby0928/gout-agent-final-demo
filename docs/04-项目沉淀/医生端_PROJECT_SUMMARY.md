...
| **AI 病史小结** | 根据语音/报告/处方自动生成过程沉淀小结，直接入档，医生可改 | 接诊后自动生成 |
...
| **查看患者资料** | 查看患者自填病史、上传报告、OCR 结果与录音识别字段 | 患者端展示资料已提交/已入档 |
| **修正档案** | 医生在接诊/档案编辑时直接修改任何字段，覆盖患者提交与 AI 识别内容 | 患者端档案同步更新 |
...
| voice | 语音接诊（诊间采集） | 录音 → AI 提取结构化字段 → 回填档案 |
| supplement | 补充资料 | 拍照上传化验单/病历/药盒 → OCR → 归档 |
| previsit | 看诊前资料 | 触发 AI 前汇总患者已有资料与完整度 |
| encounter | AI 诊疗辅助 | 生成诊疗方案辅助（依据/风险/建议） |
| confirm | 字段确认 / 回写 | 核对 AI 提取草稿与多来源冲突，回写档案 |
...
| AI 识别 | OCR/系统自动识别（病历/化验单/药盒） | 两端 |
| 医生补录 | 医生事后手动补充/修改 | 两端 |
| 医生确认 | 医生在 AI 诊疗辅助确认页确认的字段 | 两端 |

> 患者端展示时不显示来源标签（选项 A：去来源、去确认环节）；来源与确认状态属于内部协作信息，保留在医生端呈现。
...
| `material_archive_status` | `pending_review`（待核对）/ `archived`（已归档） |
...
同一字段存在多来源时，回写优先级为：
1. **医生端直接编辑 / 医生确认**（最高优先级，可随时覆盖其他来源）
2. **患者手动填写**（提交后直接入档）
3. **OCR 识别结果**（识别后直接入档，低置信度标注提示）
4. **录音识别 / AI 整理结果**（直接入档，医生可改）
5. **线下简表 / 历史记录**

**核心规则**：患者提交内容、OCR 识别结果、录音提取与 AI 小结均直接写入档案，医生可随时在接诊/档案编辑时修正或覆盖。唯一保留的硬门槛是 **AI 诊疗辅助的治疗建议（开药、调药、停药、开检查）——必须经医生确认执行后才成为医嘱**。
...
| 优先级 | 待办事项 | 说明 |
|--------|---------|------|
| **P0** | 主任确认「字段确认页」定位 | 当前为核对/回写页；需确认是否保留逐字段确认或改为批量回写 |
...
| 多来源字段冲突 | 自动覆盖 | 医生端编辑 > 患者提交 > OCR > AI，前者可随时覆盖 |
...
| 患者/AI 内容直接入档 | 降低操作负担；医生保留随时修正能力 |
