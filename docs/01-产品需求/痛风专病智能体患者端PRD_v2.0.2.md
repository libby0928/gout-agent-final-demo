...
| 整合来源 | 患者端首页状态规范 v1.1、上传报告状态反馈规范 v1.0、Demo 设计规范 AI 执行版、不同端口资料覆盖原则 v1.0.0、医生端患者端交互交叉确认断裂点反馈、字段字典 v1.1.1 |
...
| 区域 | 内容 |
| --- | --- |
| 上方状态栏 | 标题「基本信息已提交」、说明「可以继续补充痛风病史和化验报告，医生接诊时会看到这些内容。」 |
...
| 区域 | 内容 |
| --- | --- |
| 上方状态栏 | 标题「等待复诊」、说明「医生已为您安排下次复诊，建议提前完成复查准备。」 |
...
| 区域 | 内容 |
| --- | --- |
| 上方状态栏 | 标题「今日复诊」、说明「今天是建议复诊日，到院后请扫描医生出示的接诊码。」 |
...
| 区域 | 内容 |
| --- | --- |
| 上方状态栏 | 标题「复诊已逾期」、说明「原复诊日期已过，建议尽快联系医生确认新的复诊安排。」 |
...
| 区域 | 内容 |
| --- | --- |
| 上方状态栏 | 标题「暂无复诊需求」、说明「医生已确认当前无需复诊，日常有问题可以继续咨询或查看病历夹。」 |
...
| 区域 | 内容 |
| --- | --- |
| 上方状态栏 | 标题「暂无复诊安排」、说明「系统暂未记录医生下发的复诊计划，后续如有安排会同步到这里。」 |
...
患者端只有一个首页壳子，首页根据患者当前状态切换状态卡内容。所有轻量任务（病史补充、报告上传、健康打卡、饮食问答）在首页对话框内完成，不跳转独立页面。只有以下三类强任务允许跳转独立页面：扫码、联系医生、病历夹总览。上传报告只允许调起系统相机、相册或文件选择器，选择完成后回到当前对话框继续识别和追问。
...
点击问题卡、建议问题、饮食问答等轻量操作时，界面必须先追加一条患者发送的消息，再追加 AI 回复，不能直接展开一段静态说明，也不能跳转到新页面。AI 角色统一使用左侧小助手图标和白色气泡，不使用文字「AI」头像，不把 AI 回复署名成"阿福"。
...
首页及对话主界面底部快捷胶囊在所有首页状态下统一固定为四个，不随患者状态增减，位置和视觉保持不变：
...
- 病历夹是独立病情总览页，不使用首页底部四个胶囊，也不放底部快捷入口
...
健康打卡不是独立设置页，也不是固定任务列表。患者首次点击底部「健康打卡」时，AI 在当前对话框内用气泡询问「你想先记录哪些每日任务？」并给出轻量胶囊选项（如「饮水」「用药」「疼痛红肿」「饮食」「运动」「自定义」）。患者选择后，AI 生成今日打卡任务，并在当前对话里展示可勾选的任务摘要。

后续再次点击「健康打卡」时，AI 优先展示今天已有任务和完成状态，同时提供一个轻量「调整任务」胶囊。健康打卡只有在患者主动点击底部「+」时才打开附件面板；默认不打开上传报告面板。
...
同一页面、同一视觉层级里，相同功能只能出现一次。底部全局快捷胶囊已经覆盖「上传报告、健康打卡、饮食问答、联系医生」时，页面内容区不得再放同名大卡片或同名强按钮。若某个流程正在进行（如 AI 正在追问补充化验报告），可以在当前 AI 气泡下出现一次任务级操作；任务完成或跳过后，该局部入口不再重复保留。
...
两端统一来源标记体系为六类，同一份档案在患者端和医生端的来源描述一致：
...
当前 V1.0 需求默认复诊提醒可以来自医生端维护、系统规则生成或患者自填补充，但患者端展示必须明确"提醒不是预约"。待确认：患者自填复诊日期是否允许直接在患者端生效为提醒，还是必须经过医生端确认后才能展示为下次复诊提醒。
...
待确认：V1.0 是否仍在患者端首页固定展示健康打卡和饮食问答。
...
| 底部快捷入口 | 所有首页状态底部固定四个快捷胶囊：联系医生、上传报告、健康打卡、饮食问答 |
...
| 来源标记 | 两端统一六类：患者自填、患者上传、录音识别、AI 识别、医生补录、医生确认 |
| 状态枚举 | 均使用患者端字段字典中的枚举 |
...
| 患者端首页状态规范 v1.1 | `docs/03-原型与规范/患者端首页状态规范_v1.1_20260713.md` | S1-S6-B 状态定义、状态卡视觉及扫码成功消息承接规则 |
